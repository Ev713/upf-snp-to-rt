...
class MAProblemSwapper(ProblemSwapper):
    ...
    def separate_linear_from_boolean_effects_and_preconditions(self):
        linear_action_map = self.createEmptyActionMap()
        boolean_action_map = self.createEmptyActionMap()
        for agent in self.action_map:
            for action in self.action_map[agent]:
                for prec in self.action_map[agent][action]['precs']:
                    prec_parsers = [
                        (parse_comparison_precondition, linear_action_map),
                        (parse_simple_boolean_precondition, boolean_action_map),
                    ]
                ...
                for eff in self.action_map[agent][action]['effs']:
                    eff_parsers = [
                        (parse_simple_change_effect, linear_action_map),
                        (parse_simple_boolean_assignment_effect, boolean_action_map),
                    ]

                    for func, action_map in eff_parsers:
                        try:
                            parsed_eff = func(eff)
                            action_map[agent][action]['effs'].append(parsed_eff)
                            break
                        except Exception:
                            continue
                    else:
                        raise ValueError(f"Impossible to compile effect {eff} to numeric STRIPS.")
        return linear_action_map, boolean_action_map
...
def create_linear_preconditions_table(linear_actions_map):
    column_counter = defaultdict(int)
    columns = ['agent', 'action', 'precondition_index', 'operator', 'value', 'args']
    for agent, actions in linear_actions_map.items():
        for action, data in actions.items():
            for lin_prec, _, _ in data['precs']:
    ...
    for agent, actions in linear_actions_map.items():
        for action, data in actions.items():
            for idx, (lin_prec, operator, value) in enumerate(data['precs']):
...
class SNP_RT_Transformer(CompilerMixin, Engine):
    ...
    def transform(self, problem: Problem) -> Problem:
        ...
        linear_actions_map, boolean_action_map = self.swapper.separate_linear_from_boolean_effects_and_preconditions()

        df_precs = create_linear_preconditions_table(linear_actions_map)
        ...
        for idx, row in df_effs.iterrows():
            ...
            for new_f in fluent_vector_dict:
                ...
                for sub_fluent in pure_vector:
                    if remove_counter_suffix(sub_fluent) == target_fluent and pure_vector[sub_fluent]!=0:
                        is_changed = True
                if not is_changed:
                    continue
